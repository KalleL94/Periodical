...
def summarize_month_for_person(
    ...
    return {
        ...
        "payslip": payslip,
        # Per-row manual adjustments, so the views can mark an aggregate that no
        # longer matches the per-day rows beneath it (see route_override_deltas).
...
def build_calendar_grid_for_month(
    ...
    def _attach_coworkers(day_data: dict, source_day: dict, holder_position: int) -> None:
        ...
        # "OT" is passed through as-is so the matching is time-based: an overtime
        # shift is not necessarily worked at the hours of the shift it replaced.
        # A swap needs no special handling either, actual_shift already carries
        # the swapped code.
